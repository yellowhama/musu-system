[CmdletBinding()]
param(
    [string]$AiUrl = $env:MUSU_NURIKUN_INTEGRATION_AI_URL,
    [string]$Model = $env:MUSU_NURIKUN_INTEGRATION_MODEL,
    [switch]$Json,
    [switch]$ProbeOnly
)
...
function Get-ActionableFix([hashtable]$Result) {
    ...
    if ($issueCodes -contains 'ollama_host_unspecified_bind_address') {
        return "OLLAMA_HOST is set to a bind address (0.0.0.0/::) that clients cannot target. Set MUSU_NURIKUN_INTEGRATION_AI_URL to http://127.0.0.1:11434/v1 or set OLLAMA_HOST to a reachable host."
    }
    if ($issueCodes -contains 'ollama_not_installed') {
        return "No common ollama.exe install path was found. Install Ollama or set MUSU_NURIKUN_INTEGRATION_AI_URL to another reachable OpenAI-compatible endpoint."
    }
    ...
    if ($issueCodes -contains 'loopback_connection_refused') {
        return "Loopback probes were refused. Start a local Ollama-compatible server or set MUSU_NURIKUN_INTEGRATION_AI_URL explicitly."
    }
    if ($issueCodes -contains 'missing_required_model') {
        return "The endpoint is reachable but the required nurikun chat model is missing. Set MUSU_NURIKUN_INTEGRATION_MODEL to an available chat model or pull the expected model."
    }
    return "Set MUSU_NURIKUN_INTEGRATION_AI_URL or start a local Ollama-compatible server."
}
...
$env:MUSU_NURIKUN_INTEGRATION_AI_URL = $AiUrl
if (-not $env:MUSU_NURIKUN_INTEGRATION_MODEL) {
    $env:MUSU_NURIKUN_INTEGRATION_MODEL = $requiredModel
}
if ($ProbeOnly) {
...
if ($LASTEXITCODE -ne 0) {
    ...
    $result.actionable_fix = "Inspect test output and verify the endpoint supports nurikun triage plus response generation."
...
$result.test_output = $testOutput.Trim()
$result.message = "nurikun real integration passed"
Emit-Result $result 0
